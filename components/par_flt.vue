...
const conv=convSet();
  ...
  export default {
    ...
    data () {
      return {
        parValue: "",
        parInt: NaN,//на самом деле флоат
        parValid: 0,
        parMin: -Number.MAX_VALUE,
        parMax: Number.MAX_VALUE,
        parInputState: false
      }
    },
    mounted(){
    ...
    methods:{
      parInput(val){
        ...
        let ldot=lval.indexOf(".");
        if(ldot>-1) lval=lval.substring(0,ldot) +"."+lval.substring(ldot+1).replace(/\./gm,"");
        this.parValue=lval;
        this.parIsChanged();
        return this.parValue;
      },
      parIsChanged(){
        let lint=parseFloat(this.parValue);
        if(this.parInt!=lint){
          this.parInt=lint;
          this.parEmit();
        }
    ...
    computed:{
      ...
      parWarning(){
        let lret="Любое число. ";
        if(this.parMin>-Number.MAX_VALUE) lret=lret+"Минимум: " + this.parMin;
        if(this.parMax<Number.MAX_VALUE) lret=lret+" Максимум: " + this.parMax;
        return lret;
      },
    }
  }
